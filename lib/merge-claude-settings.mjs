...
const hasBashPrHook = settings.hooks.PreToolUse.some((entry) =>
  entry.matcher === 'Bash' &&
  (entry.hooks || []).some((h) => h.command?.includes('block-ai-pr-attribution.sh'))
);
if (!hasBashPrHook) {
  settings.hooks.PreToolUse.push({
    matcher: 'Bash',
    hooks: [
      {
        type: 'command',
        command: '${CLAUDE_PROJECT_DIR}/.claude/hooks/block-ai-pr-attribution.sh',
      },
    ],
  });
}
